 ...
 */
 (function() {
	...
	var viewEl = document.querySelector('.ro'),
		gridEl = viewEl.querySelector('.co'),
		items = [].slice.call(gridEl.querySelectorAll('.listing')),
		basket;
	...
	CompareBasket.prototype._createItemPreview = function(item) {
		...
		var productImageEl = item.querySelector('.kotaks').cloneNode(true);
		preview.appendChild(productImageEl);

		preview.appendChild(removeCtrl);

		// var productInfo = item.querySelector('.container-list-second').innerHTML;
	...
	CompareBasket.prototype._compareItems = function() {
		...
		for(var i = 0; i < this.totalItems; ++i) {
			...
			// console.log(compareItemWrapper);
		}
